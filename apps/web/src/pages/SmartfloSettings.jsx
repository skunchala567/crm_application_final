...
export default function SmartfloSettings({onMessage}){
  const [form,setForm]=useState({accountName:'Tata Smartflo',email:'',password:'',permanentToken:'',clearPermanentToken:false,defaultDid:'',defaultDepartmentId:'',recordCalls:true,isActive:true});
  ...
  async function load(){const result=await api('/smartflo/config');setForm(current=>({...current,...result.data,password:'',permanentToken:'',clearPermanentToken:false}));if(result.data.configured){const numberResult=await api('/smartflo/numbers');setNumbers(array(numberResult.data));const departmentResult=await api('/smartflo/departments');setDepartments(array(departmentResult.data));}}
