...
export function createJodoWebhookRoutes(pool){
  ...
  router.post('/',async(req,res,next)=>{try{
    ...
    try{
      ...
      const [[lead]]=await connection.execute(`SELECT l.id,l.stage_id,b.application_stage_id FROM crm_leads l JOIN mse_hrm_branches b ON b.id=l.branch_id WHERE l.jodo_order_id=? AND l.deleted_at_utc IS NULL ORDER BY l.id DESC LIMIT 1 FOR UPDATE`,[orderId]);
