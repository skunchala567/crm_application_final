...
async function integration(pool,organizationId,required=true){
  ...
  const cfg=parse(row.config);return {...row,...cfg,email:cfg.emailEncrypted?decryptToken(cfg.emailEncrypted,getMasterKey()):'',password:cfg.passwordEncrypted?decryptToken(cfg.passwordEncrypted,getMasterKey()):'',permanentToken:cfg.permanentTokenEncrypted?decryptToken(cfg.permanentTokenEncrypted,getMasterKey()):''};
}
