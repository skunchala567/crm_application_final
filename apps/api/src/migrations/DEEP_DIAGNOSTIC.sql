...
-- This mimics: SELECT id FROM whatsapp_templates WHERE integration_id = ? AND LOWER(template_name) = ? AND deleted_at IS NULL
...
-- 2. Check if there are ANY templates with this name (across all integrations/orgs)
...
-- 3. Show all integrations and their template counts
