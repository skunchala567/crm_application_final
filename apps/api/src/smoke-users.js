...
try {
  ...
  const user = users[0];
  const [branches] = await connection.execute(`SELECT branch_id AS id FROM mse_hrm_user_branches WHERE user_id=? UNION SELECT branch_id AS id FROM mse_hrm_app_users WHERE id=? AND branch_id IS NOT NULL`, [user.id,user.id]);
