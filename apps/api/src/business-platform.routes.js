...
const brandLogo = (value) => {
  ...
  if (/^https:\/\/[^\s"'<>]+$/i.test(raw)) return raw;
  if (/^data:image\/(png|jpeg|jpg|gif|webp);base64,[A-Za-z0-9+/=]+$/i.test(raw))
    return raw;
  return null;
};
...
import {
  LEAD_FIELD_CATALOGUE,
  CATALOGUE_BY_KEY,
  SOURCE_LABELS,
  CONFIG_FIELD_PREFIX,
} from "./lead-field-catalogue.js";
...
import {
  ...
  describeRule,
} from "./lead-duplicate-rule.js";

export function createBusinessPlatformRoutes(
  pool,
  authenticate,
  requireCrmAccess,
  requireUserAdmin,
  hashPassword,
  integrationHubService = null,
) {
  ...
  async function accessibleUnit(req, unitId, manage = false) {
    if (isAdmin(req.user)) {
      const [[unit]] = await pool.execute(
        `SELECT * FROM crm_business_units WHERE id=? AND is_active=TRUE`,
        [unitId],
      );
      return unit;
    }
    ...
    if (
      manage &&
      !["admin", "manage", "contribute"].includes(unit?.accessLevel)
    )
      return null;
    return unit;
  }
  ...
  router.get("/business-units", async (req, res) => {
    ...
    res.json({
      data: rows.map((row) => ({
        ...
        helpSubtitle: row.help_subtitle,
        compatibilityMode: row.compatibility_mode,
        isDefault: Boolean(row.is_default),
        isActive: Boolean(row.is_active),
        deletionPasswordConfigured: Boolean(row.deletion_password_hash),
        accessLevel: row.accessLevel || "admin",
        leadCount:
          Number(row.dynamicLeadCount || 0) + Number(row.legacyLeadCount || 0),
      })),
    });
  });
    ...
    try {
      ...
      const [result] = await connection.execute(
        ...
        [
          code,
          name,
          text(req.body.brandTitle, 80),
          text(req.body.brandSubtitle, 120),
          brandLogo(req.body.brandLogo),
          helpUrl(req.body.helpUrl),
          text(req.body.helpTitle, 80),
          text(req.body.helpSubtitle, 160),
          text(req.body.industryType, 80) || "General",
          text(req.body.description, 500),
          text(req.body.iconKey, 50) || "building",
          /^#[0-9a-f]{6}$/i.test(req.body.color || "")
            ? req.body.color
            : "#4A4FB1",
          text(req.body.deletionPassword, 200)
            ? hashPassword(text(req.body.deletionPassword, 200))
            : null,
          Number(req.user.id),
        ],
      );
      ...
      await connection.execute(
        `INSERT INTO crm_user_business_units(user_id,business_unit_id,access_level,is_default) VALUES(?,?,'admin',FALSE)`,
        [Number(req.user.id), unitId],
      );
      await connection.execute(
      ...
      const [channelCategoryResult] = await connection.execute(
        `INSERT INTO crm_business_channel_categories(business_unit_id,category_key,display_name) VALUES(?,'primary','Primary')`,
        [unitId],
      );
      ...
      const [campaignCategoryResult] = await connection.execute(
        `INSERT INTO crm_business_campaign_categories(business_unit_id,category_key,display_name) VALUES(?,'general','General')`,
        [unitId],
      );
      await connection.execute(
      ...
      for (const [key, label, type, required, searchable, position, width] of [
        ["name", "Name", "text", 1, 1, 1, 220],
        ["phone", "Phone", "phone", 0, 1, 2, 150],
        ["email", "Email", "email", 0, 1, 3, 220],
        ["source", "Source", "single_select", 0, 0, 4, 160],
        ["status", "Status", "single_select", 0, 0, 5, 130],
      ])
        await connection.execute(
      ...
      const [pipelineResult] = await connection.execute(
        `INSERT INTO crm_metadata_pipelines(business_unit_id,pipeline_key,display_name,is_default)
         VALUES (?,'default','Default Pipeline',TRUE)`,
        [unitId],
      );
      ...
      const [leadPipelineResult] = await connection.execute(
        `INSERT INTO crm_lead_pipelines(business_unit_id,pipeline_key,display_name,description,is_default,position)
         VALUES (?,'main','Main pipeline','The stages leads move through in this business unit.',TRUE,1)`,
        [unitId],
      );
      const leadPipelineId = Number(leadPipelineResult.insertId);
      for (const [key, label, type, color, position] of [
        ["new", "New", "open", "#555AB1", 1],
        ["qualified", "Qualified", "open", "#3B82F6", 2],
        ["won", "Won", "won", "#258268", 3],
        ["lost", "Lost", "lost", "#B84848", 4],
      ]) {
        ...
        await connection.execute(
          `INSERT INTO crm_lead_substages(stage_id,substage_code,display_name,position)
           VALUES(?,?,?,1)`,
          [
            Number(sharedStageResult.insertId),
            `bu${unitId}__${key}_pending`,
            `${label} - Pending`,
          ],
        );
      }
      const [workflowResult] = await connection.execute(
        `INSERT INTO crm_operation_workflows(business_unit_id,workflow_key,display_name,entity_label,is_default)
         VALUES (?,'default','Progress & MOM Tracker','Action item',TRUE)`,
        [unitId],
      );
      const workflowId = Number(workflowResult.insertId);
      for (const [key, label, type, color, position] of [
        ["open", "Open", "open", "#F04420", 1],
        ["in_progress", "In progress", "open", "#3327EF", 2],
        ["hold", "Hold", "on_hold", "#4A4FB1", 3],
        ["completed", "Completed", "completed", "#258268", 4],
      ...
      res
        ...
        .json({
          id: unitId,
          message:
            "Business unit created with starter lead and tracker metadata",
        });
    } catch (error) {
      ...
      if (error.code === "ER_DUP_ENTRY")
        return res
          .status(409)
          .json({ message: "A business unit with this code already exists" });
      throw error;
    } finally {
      connection.release();
    }
  ...
  router.put(
    ...
    async (req, res) => {
      const unitId = Number(req.params.id);
      const [[unit]] = await pool.execute(
        `SELECT is_default AS isDefault FROM crm_business_units WHERE id=?`,
        [unitId],
      );
      if (!unit)
        return res.status(404).json({ message: "Business unit not found" });
      if (unit.isDefault && !req.body.isActive)
        return res
          .status(400)
          .json({
            message: "The default School Admissions unit cannot be deactivated",
          });
      await pool.execute(
        `UPDATE crm_business_units SET is_active=?,updated_by_user_id=? WHERE id=?`,
        [req.body.isActive ? 1 : 0, Number(req.user.id), unitId],
      );
      res.json({
        message: `Business unit marked ${req.body.isActive ? "active" : "inactive"}`,
      });
    },
  );
  ...
  router.delete("/business-units/:id", requireUserAdmin, async (req, res) => {
    const unitId = Number(req.params.id);
    const [[unit]] = await pool.execute(
      `SELECT id,is_default AS isDefault,display_name AS displayName FROM crm_business_units WHERE id=?`,
      [unitId],
    );
    if (!unit)
      return res.status(404).json({ message: "Business unit not found" });
    if (unit.isDefault)
      return res
        .status(409)
        .json({ message: "The default Business Unit cannot be deleted" });
    ...
    const linked =
      Number(usage.leads) +
      Number(usage.dynamicLeads) +
      Number(usage.trackerRecords);
    if (linked)
      return res
        .status(409)
        .json({
          message: `Cannot delete ${unit.displayName} because it is linked to ${linked} lead or Tracker record${linked === 1 ? "" : "s"}`,
        });
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();
      await connection.execute(
        `DELETE ss FROM crm_lead_substages ss JOIN crm_lead_stages s ON s.id=ss.stage_id WHERE s.business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_lead_stages WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_business_sources WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_business_channels WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_business_channel_categories WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_business_campaigns WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(
        `DELETE FROM crm_business_campaign_categories WHERE business_unit_id=?`,
        [unitId],
      );
      await connection.execute(`DELETE FROM crm_business_units WHERE id=?`, [
        unitId,
      ]);
      await connection.commit();
      res.json({
        message: "Business unit and its unused configuration deleted",
      });
    } catch (error) {
      await connection.rollback();
      if (error.code === "ER_ROW_IS_REFERENCED_2")
        return res
          ...
          .json({
            message:
              "This Business Unit still has linked application records and cannot be deleted",
          });
      throw error;
    } finally {
      connection.release();
  ...
  router.get("/business-units/:id/config", async (req, res) => {
    const unitId = Number(req.params.id),
      unit = await accessibleUnit(req, unitId);
    if (!unit)
    ...
    const [
      ...
      pool.execute(
        `SELECT id,field_key AS fieldKey,display_name AS displayName,field_type AS fieldType,placeholder,help_text AS helpText,options_json AS options,options_section_id AS optionsSectionId,options_section_level AS optionsSectionLevel,validation_json AS validation,is_system AS isSystem,is_required AS isRequired,is_filterable AS isFilterable,filter_control AS filterControl,is_searchable AS isSearchable,is_importable AS isImportable,is_import_required AS isImportRequired,import_header AS importHeader,import_sample_value AS importSampleValue,show_in_list AS showInList,position,column_width AS columnWidth,is_active AS isActive FROM crm_metadata_fields WHERE business_unit_id=? ORDER BY module_key,position`,
        [unitId],
      ),
      pool.execute(
        `SELECT id,form_key AS formKey,display_name AS displayName,form_type AS formType,sections_json AS sections,is_default AS isDefault,is_active AS isActive FROM crm_metadata_forms WHERE business_unit_id=? ORDER BY display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT id,form_key AS formKey,display_name AS displayName,description,default_branch_id AS defaultBranchId,default_stage_id AS defaultStageId,default_substage_id AS defaultSubstageId,default_source_id AS defaultSourceId,default_channel_id AS defaultChannelId,default_campaign_id AS defaultCampaignId,default_owner_employee_id AS defaultOwnerEmployeeId,field_schema_json AS fieldSchema,settings_json AS settings,success_message AS successMessage,redirect_url AS redirectUrl,payment_required AS paymentRequired,payment_amount AS paymentAmount,is_active AS isActive FROM crm_public_enquiry_forms WHERE business_unit_id=? AND ${enquiryScope.sql} ORDER BY display_name`,
        [unitId, ...enquiryScope.params],
      ),
      pool.execute(
        `SELECT id,pipeline_key AS pipelineKey,display_name AS displayName,description,entity_label_singular AS entityLabelSingular,entity_label_plural AS entityLabelPlural,is_default AS isDefault,is_active AS isActive FROM crm_metadata_pipelines WHERE business_unit_id=? ORDER BY is_default DESC,display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT ps.id,ps.pipeline_id AS pipelineId,ps.stage_key AS stageKey,ps.display_name AS displayName,ps.stage_type AS stageType,ps.color_code AS color,ps.position,ps.is_active AS isActive FROM crm_metadata_pipeline_stages ps JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id WHERE p.business_unit_id=? ORDER BY ps.pipeline_id,ps.position`,
        [unitId],
      ),
      pool.execute(
        `SELECT id,workflow_key AS workflowKey,display_name AS displayName,entity_label AS entityLabel,description,form_schema_json AS formSchema,is_default AS isDefault,is_active AS isActive FROM crm_operation_workflows WHERE business_unit_id=? ORDER BY is_default DESC,display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT os.id,os.workflow_id AS workflowId,os.stage_key AS stageKey,os.display_name AS displayName,os.stage_type AS stageType,os.color_code AS color,os.position,os.checklist_json AS checklist,os.is_active AS isActive FROM crm_operation_stages os JOIN crm_operation_workflows ow ON ow.id=os.workflow_id WHERE ow.business_unit_id=? ORDER BY os.workflow_id,os.position`,
        [unitId],
      ),
      pool.execute(
      ...
      // every channel.
      pool.query(
        `SELECT id,display_name AS displayName,channel_id AS channelId FROM crm_lead_sources WHERE is_active=TRUE ORDER BY display_name`,
      ),
      pool.query(
        `SELECT id,display_name AS displayName FROM crm_lead_channels WHERE is_active=TRUE ORDER BY display_name`,
      ),
      pool.query(
        `SELECT id,display_name AS displayName FROM crm_campaigns WHERE is_active=TRUE ORDER BY display_name`,
      ),
      pool.execute(
      ...
      pool.query(
        `SELECT academic_year AS academicYear,display_name AS displayName FROM crm_academic_years WHERE is_active=TRUE ORDER BY academic_year DESC`,
      ),
    ]);
    ...
    if (unit) {
      const [[legacyStages], [legacySubstages]] = await Promise.all([
        pool.execute(
          `SELECT id,pipeline_id AS pipelineId,name AS stageKey,display_name AS displayName,'open' AS stageType,color_code AS color,position,is_active AS isActive,requires_followup AS requiresFollowup,is_admission_stage AS isAdmissionStage FROM crm_lead_stages WHERE business_unit_id=? ORDER BY position,display_name`,
          [unitId],
        ),
        pool.execute(
          `SELECT ss.id,ss.stage_id AS stageId,s.pipeline_id AS pipelineId,ss.substage_code AS substageKey,ss.display_name AS displayName,ss.position,ss.is_active AS isActive FROM crm_lead_substages ss JOIN crm_lead_stages s ON s.id=ss.stage_id WHERE s.business_unit_id=? ORDER BY s.position,ss.position,ss.display_name`,
          [unitId],
        ),
      ]);
      ...
      const [leadPipelines] = await pool.execute(
        `SELECT id,pipeline_key AS pipelineKey,display_name AS displayName,description,
           ...
           FROM crm_lead_pipelines WHERE business_unit_id=? ORDER BY position,display_name`,
        [unitId],
      );
      ...
      const [metadataSubstages] = await pool.execute(
        `SELECT ss.id,ss.stage_id AS stageId,ss.substage_key AS substageKey,ss.display_name AS displayName,ss.position,ss.is_active AS isActive
         ...
         WHERE p.business_unit_id=? ORDER BY ps.position,ss.position,ss.display_name`,
        [unitId],
      );
    ...
    res.json({
      unit: {
        ...
        industryType: unit.industry_type,
        compatibilityMode: unit.compatibility_mode,
        color: unit.color_code,
      },
      ...
      duplicateRuleDescription: describeRule(
        normalizeDuplicateRule(unit.duplicate_rule_json),
      ),
      duplicateFields: normalize(fields)
        ...
        .map((field) => {
          ...
          if (
            !field.isSystem &&
            ["single_select", "text", "number", "phone", "email"].includes(
              field.fieldType,
            )
          ) {
            return {
      ...
      forms: normalize(forms),
      pipelines: resolvedPipelines,
      pipelineStages: resolvedPipelineStages,
      leadSubstages: normalize(leadSubstages),
      enquiryForms: normalize(enquiryForms),
      branches: normalize(branches),
      sources: normalize(sources),
      channels: normalize(channels),
      campaigns: normalize(campaigns),
      employees: normalize(employees),
      academicYears: normalize(academicYears),
      workflows: normalize(workflows),
      operationStages: normalize(operationStages),
      modules: normalize(modules),
    });
  });

  router.get(
    "/business-units/:id/database-tables",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
      ...
      for (const table of tables) {
        ...
        const orderColumn =
          [
            ...
            "updated_at",
            "id",
          ].find((column) => columns.includes(column)) || columns[0];
        const [[countRow]] = await pool.query(
          `SELECT COUNT(*) AS count FROM ${escapeIdentifier(tableName)} WHERE business_unit_id=?`,
          [unitId],
        );
        const [rows] = await pool.query(
          `SELECT * FROM ${escapeIdentifier(tableName)} WHERE business_unit_id=? ORDER BY ${escapeIdentifier(orderColumn)} DESC LIMIT 5`,
          [unitId],
        );
        data.push({
          tableName,
      ...
      res.json({
        businessUnit: { id: unitId, name: unit.display_name || unit.name },
        tables: data,
      });
    },
  );
    ...
    async (req, res) => {
      ...
      if (Number(smartfloSchema.count) < 2)
        return res
          ...
          .json({
            message:
              "Run database migrations 052_smartflo_telephony.sql and 053_smartflo_ivr_mapping.sql before configuring Tata Smartflo",
          });
      const name = text(req.body.name, 150),
        shortName = text(req.body.shortName, 50) || name;
      if (!name)
      ...
      try {
        const [result] = await pool.execute(
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [
            ...
            req.body.jodoPaymentEnabled ? 1 : 0,
            text(req.body.jodoApiKey, 255),
            text(req.body.jodoSecretKey, 255),
            text(req.body.jodoCollectorCode, 100),
            text(req.body.jodoBaseUrl, 255),
            text(req.body.jodoAuthHeader, 512),
            req.body.applicationAmount
              ? Number(req.body.applicationAmount)
              : null,
            req.body.applicationStageId
              ? Number(req.body.applicationStageId)
              : null,
            text(req.body.applicationPaymentComponent, 120) || "Payable Amount",
            text(req.body.callerdeskDidId, 100),
            text(req.body.callerdeskDidNumber, 30),
            text(req.body.callerdeskCallGroup, 120),
            req.body.callerdeskInboundEnabled === false ? 0 : 1,
            req.body.callerdeskOutboundEnabled === false ? 0 : 1,
            text(req.body.smartfloDidId, 100),
            text(req.body.smartfloDidNumber, 30),
            text(req.body.smartfloIvrId, 100),
            text(req.body.smartfloIvrName, 150),
            text(req.body.smartfloDepartmentId, 100),
            req.body.smartfloInboundEnabled === false ? 0 : 1,
        ...
        if (error.code === "ER_NO_DEFAULT_FOR_FIELD")
          return res
            ...
            .json({
              message:
                "This branch table has additional required columns. Please create the branch in the master branch setup, then configure payment details here.",
            });
        throw error;
      }
    },
    ...
    async (req, res) => {
      ...
      const [result] = await pool.execute(
       ...
       WHERE id=?`,
        [
          name,
          text(req.body.shortName, 50) || name,
          req.body.isActive === false ? 0 : 1,
          req.body.jodoPaymentEnabled ? 1 : 0,
          text(req.body.jodoApiKey, 255),
          text(req.body.jodoSecretKey, 255),
          ...
          req.body.applicationStageId
            ? Number(req.body.applicationStageId)
            : null,
          text(req.body.applicationPaymentComponent, 120) || null,
          text(req.body.callerdeskDidId, 100),
          text(req.body.callerdeskDidNumber, 30),
          text(req.body.callerdeskCallGroup, 120),
          req.body.callerdeskInboundEnabled === false ? 0 : 1,
  ...
  router.post(
    "/business-units/:id/fields",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        displayName = text(req.body.displayName, 150),
        fieldKey = slug(req.body.fieldKey || displayName);
      ...
      const fieldType = allowedTypes.includes(req.body.fieldType)
        ? req.body.fieldType
        : "text";
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_metadata_fields WHERE business_unit_id=? AND module_key='leads'`,
        [unitId],
      );
      let optionSource;
      try {
        optionSource = await optionSourceFor(unitId, req.body);
      } catch (error) {
        ...
        const [result] = await pool.execute(
         ...
         VALUES (?,'leads',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [
            ...
            text(req.body.helpText, 500),
            JSON.stringify(
              Array.isArray(req.body.options) ? req.body.options : [],
            ),
            optionSource.sectionId,
            optionSource.level,
            JSON.stringify(
              req.body.validation && typeof req.body.validation === "object"
                ? req.body.validation
                : {},
            ),
            req.body.isRequired ? 1 : 0,
            req.body.isFilterable === false ? 0 : 1,
            text(req.body.filterControl, 40) || "contains",
            ...
            Math.max(100, Math.min(400, Number(req.body.columnWidth) || 180)),
          ],
        );
        res
          .status(201)
          .json({ id: Number(result.insertId), message: "Custom field added" });
      } catch (error) {
        if (error.code === "ER_DUP_ENTRY")
          return res
            .status(409)
            .json({ message: "This field key already exists" });
        throw error;
      }
    },
  );
  ...
  async function optionSourceFor(unitId, body) {
    ...
    if (!section)
      throw Object.assign(
        new Error(
          "That configuration section is not part of this business unit",
        ),
        { status: 400 },
      );
    const level =
      section.sectionType === "hierarchy" &&
      body.optionsSectionLevel === "child"
        ? "child"
        : "parent";
  ...
  async function unusedFieldKey(unitId, base) {
    const slug =
      String(base)
        ...
        .replace(/[^a-z0-9_]+/g, "_")
        .replace(/^_+|_+$/g, "")
        .slice(0, 60) || `${CONFIG_FIELD_PREFIX}field`;
    const [rows] = await pool.execute(
      `SELECT field_key AS fieldKey FROM crm_metadata_fields
        WHERE business_unit_id=? AND module_key='leads' AND field_key LIKE ?`,
      [unitId, `${slug}%`],
    );
  ...
  async function configSectionCatalogue(unitId) {
    ...
    const [bound] = await pool.execute(
      `SELECT options_section_id AS sectionId, options_section_level AS level
        ...
        WHERE business_unit_id=? AND module_key='leads' AND options_section_id IS NOT NULL`,
      [unitId],
    );
    const used = new Set(
      bound.map((row) => `${row.sectionId}:${row.level || "parent"}`),
    );
      ...
      const levels =
        section.sectionType === "hierarchy"
          ? [
              ...
              [
                "child",
                `${section.displayName} \u2192 ${section.childLabel || "sub-values"}`,
              ],
            ]
          : [["parent", section.displayName]];
      return levels
        ...
        .map(([level, label]) => ({
          ...
          group: section.pipelineName
            ? `Configuration \u00b7 ${section.pipelineName}`
            : "Configuration",
          width: 170,
   ...
   */
  router.get(
    "/business-units/:unitId/field-catalogue",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId);
      const [existing] = await pool.execute(
        `SELECT field_key AS fieldKey FROM crm_metadata_fields WHERE business_unit_id=? AND module_key='leads'`,
        [unitId],
      );
      const taken = new Set(existing.map((row) => row.fieldKey));
      res.json({
        data: [
          ...LEAD_FIELD_CATALOGUE.filter((entry) => !taken.has(entry.key)),
          ...(await configSectionCatalogue(unitId)),
        ].map((entry) => ({
          ...entry,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (!entry && requestedKey.startsWith("section:")) {
        ...
        const level =
          section.sectionType === "hierarchy" && levelText === "child"
            ? "child"
            : "parent";
        optionSource = { sectionId: Number(section.id), level };
        entry = {
          key: await unusedFieldKey(
            unitId,
            `${CONFIG_FIELD_PREFIX}${section.sectionKey}${level === "child" ? "_child" : ""}`,
          ),
          label:
            level === "child"
              ? `${section.displayName} ${section.childLabel || "sub-value"}`
              : section.displayName,
          type: "single_select",
          source: "config",
          width: 170,
        ...
        if (alreadyBound)
          return res
            .status(409)
            .json({
              message: `${alreadyBound.displayName} already reads from that configuration section`,
            });
      }
      if (!entry)
        return res.status(400).json({ message: "Unknown standard field" });
      const [[duplicate]] = await pool.execute(
        `SELECT id FROM crm_metadata_fields WHERE business_unit_id=? AND module_key='leads' AND field_key=?`,
        [unitId, entry.key],
      );
      if (duplicate)
        return res
          .status(409)
          .json({ message: `${entry.label} is already on this business unit` });
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_metadata_fields WHERE business_unit_id=? AND module_key='leads'`,
        [unitId],
      );
      ...
      const requested =
        req.body.validation && typeof req.body.validation === "object"
          ...
          : {};
      const usage =
        requested.usage && typeof requested.usage === "object"
          ? requested.usage
          : {
              leadForm: true,
              quickCreate: false,
              ...
              list: true,
              filters: true,
              search: Boolean(entry.searchable),
      ...
      const [result] = await pool.execute(
       ...
       VALUES (?,'leads',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          ...
          displayName,
          entry.type,
          text(req.body.placeholder, 200) || entry.placeholder || null,
          JSON.stringify(entry.options || []),
          optionSource?.sectionId || null,
          optionSource?.level || "parent",
          JSON.stringify({
            ...requested,
            usage,
            requiredOnLeadForm: Boolean(req.body.isRequired),
          }),
          optionSource ? 0 : 1,
          req.body.isRequired ? 1 : 0,
          req.body.isFilterable === false ? 0 : 1,
          text(req.body.filterControl, 40) ||
            entry.filterControl ||
            (["single_select", "user"].includes(entry.type)
              ? "single_select"
              : ["datetime", "date"].includes(entry.type)
                ? "date_range"
                : "contains"),
          req.body.isSearchable ? 1 : 0,
          req.body.isImportable === false ? 0 : 1,
          ...
          Math.max(
            100,
            Math.min(400, Number(req.body.columnWidth) || entry.width || 180),
          ),
        ],
      );
      res
        .status(201)
        .json({ id: Number(result.insertId), message: `${entry.label} added` });
    },
  );

  router.put(
    "/business-units/:unitId/fields/:id",
    requireUserAdmin,
    async (req, res) => {
      let optionSource;
      try {
        optionSource = await optionSourceFor(
      ...
      const [result] = await pool.execute(
       ...
       WHERE id=? AND business_unit_id=?`,
        [
          ...
          req.body.isImportable === false ? 0 : 1,
          req.body.isImportRequired ? 1 : 0,
          text(req.body.importHeader, 160) || text(req.body.displayName, 150),
          text(req.body.importSampleValue, 255),
          req.body.showInList === false ? 0 : 1,
          Math.max(100, Math.min(400, Number(req.body.columnWidth) || 180)),
          req.body.isActive === false ? 0 : 1,
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      const [[field]] = await pool.execute(
        `SELECT id,field_key AS fieldKey,display_name AS displayName,is_system AS isSystem FROM crm_metadata_fields WHERE id=? AND business_unit_id=?`,
        [fieldId, unitId],
      );
      if (!field)
        return res.status(404).json({ message: "Lead field not found" });
      ...
      if (essentialFields.has(field.fieldKey))
        return res
          .status(409)
          .json({
            message: `${field.displayName} is a mandatory application field and cannot be deleted`,
          });
      ...
      const sharedColumns = new Map([
        ...
        ["next_followup_at_utc", "next_followup_at_utc"],
        ["remarks", "remarks"],
      ...
      if (column) {
        ...
        const [[usage]] = await pool.execute(
          `SELECT COUNT(*) AS count FROM crm_followups f JOIN crm_leads l ON l.id=f.lead_id WHERE l.business_unit_id=? AND f.followup_type IS NOT NULL`,
          [unitId],
        );
        sharedCount = Number(usage.count);
      }
      ...
      if (linkedCount > 0)
        return res
          .status(409)
          .json({
            message: `Cannot delete ${field.displayName} because ${linkedCount} lead${linkedCount === 1 ? " contains" : "s contain"} a value for it`,
          });
      const connection = await pool.getConnection();
      try {
        await connection.beginTransaction();
        const [forms] = await connection.execute(
          `SELECT id,sections_json AS sections FROM crm_metadata_forms WHERE business_unit_id=?`,
          [unitId],
        );
        const removeField = (value) => {
          if (Array.isArray(value))
            return value
              .filter((item) => item !== field.fieldKey)
              .map(removeField);
          if (value && typeof value === "object")
            return Object.fromEntries(
              Object.entries(value)
                .filter(
                  ([key, val]) =>
                    key !== field.fieldKey && val !== field.fieldKey,
                )
                .map(([key, val]) => [key, removeField(val)]),
            );
          return value;
        };
        for (const form of forms) {
          ...
          await connection.execute(
            `UPDATE crm_metadata_forms SET sections_json=? WHERE id=?`,
            [JSON.stringify(sections), form.id],
          );
        }
        await connection.execute(
          `DELETE FROM crm_metadata_fields WHERE id=? AND business_unit_id=?`,
          [fieldId, unitId],
        );
        await connection.commit();
  ...
  const enquiryPayload = (req) => {
    ...
    return {
      ...
      defaultBranchId: req.body.defaultBranchId
        ? Number(req.body.defaultBranchId)
        : null,
      defaultStageId: req.body.defaultStageId
        ? Number(req.body.defaultStageId)
        : null,
      defaultSubstageId: req.body.defaultSubstageId
        ? Number(req.body.defaultSubstageId)
        : null,
      defaultSourceId: req.body.defaultSourceId
        ? Number(req.body.defaultSourceId)
        : null,
      defaultChannelId: req.body.defaultChannelId
        ? Number(req.body.defaultChannelId)
        : null,
      defaultCampaignId: req.body.defaultCampaignId
        ? Number(req.body.defaultCampaignId)
        : null,
      defaultOwnerEmployeeId: req.body.defaultOwnerEmployeeId
        ? Number(req.body.defaultOwnerEmployeeId)
        : null,
      fields,
      settings: {
        ...(req.body.settings && typeof req.body.settings === "object"
          ? req.body.settings
          : {}),
        defaultAcademicYear: text(
          req.body.settings?.defaultAcademicYear ||
            req.body.defaultAcademicYear,
          20,
        ),
        paymentComponentType: text(
          req.body.settings?.paymentComponentType,
          120,
        ),
      },
      successMessage:
        text(req.body.successMessage, 500) ||
  ...
  router.post(
    "/business-units/:id/enquiry-forms",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        unit = await accessibleUnit(req, unitId, true);
      ...
      if (!payload.defaultBranchId || !payload.defaultStageId)
        return res
          .status(400)
          .json({ message: "Default branch and stage are required" });
      ...
      if (!payload.settings.defaultAcademicYear)
        return res
          .status(400)
          .json({ message: "Academic year is required for enquiry forms" });
      if (!payload.fields.length)
        return res
          .status(400)
          .json({ message: "Select at least one field for the enquiry form" });
      const formKey =
        slug(req.body.formKey || `${unit.unit_code}_${payload.displayName}`) ||
        `enquiry_${Date.now()}`;
      try {
        const [result] = await pool.execute(
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [
            ...
            payload.defaultStageId,
            payload.defaultSubstageId,
            payload.defaultSourceId,
            payload.defaultChannelId,
            payload.defaultCampaignId,
            payload.defaultOwnerEmployeeId,
            JSON.stringify(payload.fields),
            JSON.stringify(payload.settings),
            payload.successMessage,
            payload.redirectUrl,
            payload.paymentRequired ? 1 : 0,
        ...
        if (error.code === "ER_DUP_ENTRY")
          return res
            .status(409)
            .json({ message: "This enquiry form key already exists" });
        throw error;
      }
    },
  );
  ...
  router.put(
    "/business-units/:unitId/enquiry-forms/:id",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      const scope = branchScopeSql(req.user, "default_branch_id");
      const [result] = await pool.execute(
        `DELETE FROM crm_public_enquiry_forms WHERE id=? AND business_unit_id=? AND ${scope.sql}`,
        [formId, unitId, ...scope.params],
      );
      if (!result.affectedRows)
        return res.status(404).json({ message: "Enquiry form not found" });
  ...
  const sourceConfigDefinitions = {
    channelCategories: {
      table: "crm_business_channel_categories",
      keyColumn: "category_key",
      label: "Channel category",
    },
    channels: {
      table: "crm_business_channels",
      keyColumn: "channel_key",
      parentColumn: "category_id",
      parentTable: "crm_business_channel_categories",
      label: "Channel",
    },
    sources: {
      table: "crm_business_sources",
      keyColumn: "source_key",
      parentColumn: "channel_id",
      parentTable: "crm_business_channels",
      label: "Source",
    },
    campaignCategories: {
      table: "crm_business_campaign_categories",
      keyColumn: "category_key",
      label: "Campaign category",
    },
    campaigns: {
      table: "crm_business_campaigns",
      keyColumn: "campaign_key",
      parentColumn: "category_id",
      parentTable: "crm_business_campaign_categories",
      label: "Campaign",
    },
  };
  ...
  router.get("/business-units/:id/source-config", async (req, res) => {
    ...
    if (unit.compatibility_mode === "legacy_school") {
      const [
        ...
        pool.query(
          `SELECT s.id,s.name code,s.display_name displayName,s.channel_id parentId,ch.display_name parentName,s.is_active isActive FROM crm_lead_sources s LEFT JOIN crm_lead_channels ch ON ch.id=s.channel_id ORDER BY s.display_name`,
        ),
        pool.query(
          `SELECT id,category_code code,display_name displayName,is_active isActive FROM crm_campaign_categories ORDER BY display_name`,
        ),
        pool.query(
          `SELECT c.id,c.campaign_code code,c.display_name displayName,c.category_id parentId,COALESCE(cc.display_name,c.category) parentName,c.is_active isActive FROM crm_campaigns c LEFT JOIN crm_campaign_categories cc ON cc.id=c.category_id ORDER BY parentName,c.display_name`,
        ),
        pool.query(
          `SELECT id,category_code code,display_name displayName,is_active isActive FROM crm_channel_categories ORDER BY display_name`,
        ),
        pool.query(
          `SELECT c.id,c.channel_code code,c.display_name displayName,c.category_id parentId,COALESCE(cc.display_name,c.category) parentName,c.is_active isActive FROM crm_lead_channels c LEFT JOIN crm_channel_categories cc ON cc.id=c.category_id ORDER BY parentName,c.display_name`,
        ),
      ]);
      ...
      return res.json({
        ...
        channelCategories,
        channels,
        manualLeadDefaults: storedDefaults,
        reEnquiryDefaults: storedDefaults.reEnquiry || {},
      });
    }
    const [
      [channelCategories],
      ...
      pool.execute(
        `SELECT id,category_key code,display_name displayName,is_active isActive FROM crm_business_channel_categories WHERE business_unit_id=? ORDER BY display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT c.id,c.channel_key code,c.display_name displayName,c.category_id parentId,cc.display_name parentName,c.is_active isActive FROM crm_business_channels c JOIN crm_business_channel_categories cc ON cc.id=c.category_id WHERE c.business_unit_id=? ORDER BY cc.display_name,c.display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT s.id,s.source_key code,s.display_name displayName,s.channel_id parentId,c.display_name parentName,s.is_active isActive FROM crm_business_sources s JOIN crm_business_channels c ON c.id=s.channel_id WHERE s.business_unit_id=? ORDER BY c.display_name,s.display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT id,category_key code,display_name displayName,is_active isActive FROM crm_business_campaign_categories WHERE business_unit_id=? ORDER BY display_name`,
        [unitId],
      ),
      pool.execute(
        `SELECT c.id,c.campaign_key code,c.display_name displayName,c.category_id parentId,cc.display_name parentName,c.is_active isActive FROM crm_business_campaigns c JOIN crm_business_campaign_categories cc ON cc.id=c.category_id WHERE c.business_unit_id=? ORDER BY cc.display_name,c.display_name`,
        [unitId],
      ),
    ]);
    const storedDefaults = parseJson(unit.manual_lead_defaults_json, {});
  ...
  router.put(
    "/business-units/:id/manual-lead-defaults",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        unit = await accessibleUnit(req, unitId, true);
      ...
      for (const key of allowed) {
        if (Object.prototype.hasOwnProperty.call(req.body || {}, key))
          selected[key] = req.body[key] ? Number(req.body[key]) : null;
      }
      ...
      if (legacy) {
        if (
          !(await ensure(
            "channelId",
            "SELECT id FROM crm_lead_channels WHERE id=? AND is_active=TRUE",
            [selected.channelId],
            "Select an active default channel",
          ))
        )
          return;
        if (
          !(await ensure(
            "sourceId",
            "SELECT id FROM crm_lead_sources WHERE id=? AND is_active=TRUE AND (? IS NULL OR channel_id=?)",
            [selected.sourceId, selected.channelId, selected.channelId],
            "The default source must belong to the selected channel",
          ))
        )
          return;
        if (
          !(await ensure(
            "campaignId",
            "SELECT id FROM crm_campaigns WHERE id=? AND is_active=TRUE",
            [selected.campaignId],
            "Select an active default campaign",
          ))
        )
          return;
        if (
          !(await ensure(
            "stageId",
            "SELECT id FROM crm_lead_stages WHERE id=? AND business_unit_id=? AND is_active=TRUE",
            [selected.stageId, unitId],
            "Select an active default stage",
          ))
        )
          return;
        if (
          !(await ensure(
            "substageId",
            "SELECT ss.id FROM crm_lead_substages ss JOIN crm_lead_stages s ON s.id=ss.stage_id WHERE ss.id=? AND s.business_unit_id=? AND ss.is_active=TRUE AND (? IS NULL OR ss.stage_id=?)",
            [selected.substageId, unitId, selected.stageId, selected.stageId],
            "The default sub-stage must belong to the selected stage",
          ))
        )
          return;
      } else {
        if (
          !(await ensure(
            "channelId",
            "SELECT id FROM crm_business_channels WHERE id=? AND business_unit_id=? AND is_active=TRUE",
            [selected.channelId, unitId],
            "Select an active default channel",
          ))
        )
          return;
        if (
          !(await ensure(
            "sourceId",
            "SELECT id FROM crm_business_sources WHERE id=? AND business_unit_id=? AND is_active=TRUE AND (? IS NULL OR channel_id=?)",
            [selected.sourceId, unitId, selected.channelId, selected.channelId],
            "The default source must belong to the selected channel",
          ))
        )
          return;
        if (
          !(await ensure(
            "campaignId",
            "SELECT id FROM crm_business_campaigns WHERE id=? AND business_unit_id=? AND is_active=TRUE",
            [selected.campaignId, unitId],
            "Select an active default campaign",
          ))
        )
          return;
        if (
          !(await ensure(
            "stageId",
            "SELECT ps.id FROM crm_metadata_pipeline_stages ps JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id WHERE ps.id=? AND p.business_unit_id=? AND ps.is_active=TRUE",
            [selected.stageId, unitId],
            "Select an active default stage",
          ))
        )
        ...
        if (
          !(await ensure(
            "substageId",
            "SELECT ss.id FROM crm_metadata_pipeline_substages ss JOIN crm_metadata_pipeline_stages ps ON ps.id=ss.stage_id JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id WHERE ss.id=? AND p.business_unit_id=? AND ss.is_active=TRUE AND (? IS NULL OR ss.stage_id=?)",
            [selected.substageId, unitId, selected.stageId, selected.stageId],
            "The default sub-stage must belong to the selected stage",
          ))
        )
      ...
      await pool.execute(
        "UPDATE crm_business_units SET manual_lead_defaults_json=?,updated_by_user_id=? WHERE id=?",
        [JSON.stringify(next), Number(req.user.id), unitId],
      );
      res.json({
        manualLeadDefaults: next,
        reEnquiryDefaults: next.reEnquiry || {},
        message: isReEnquiry
          ? "Re-enquiry defaults saved"
          : "Manual Add Lead defaults saved",
      });
    },
  );

  router.post(
    "/business-units/:id/source-config/:type",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        unit = await accessibleUnit(req, unitId, true),
        definition = sourceConfigDefinitions[req.params.type];
      if (!unit)
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      if (!definition)
        return res
          .status(404)
          .json({ message: "Configuration type not found" });
      if (unit.compatibility_mode === "legacy_school")
        return res
          .status(400)
          .json({
            message: "Use the School Admissions source configuration adapter",
          });
      const displayName = text(req.body.displayName, 150),
        key = slug(req.body.code || displayName);
      if (!displayName || !key)
      ...
      if (definition.parentColumn) {
        ...
        const [[parent]] = await pool.execute(
          `SELECT id FROM ${definition.parentTable} WHERE id=? AND business_unit_id=?`,
          [parentId, unitId],
        );
        if (!parent)
          return res
            .status(400)
            .json({ message: `${definition.label} parent is required` });
        columns.push(definition.parentColumn);
        values.push(parentId);
      }
      columns.push(definition.keyColumn, "display_name");
      values.push(key, displayName);
      try {
        const [result] = await pool.execute(
        ...
        if (error.code === "ER_DUP_ENTRY")
          return res
            .status(409)
            .json({
              message: `A ${definition.label.toLowerCase()} with this name already exists`,
            });
        throw error;
      }
    },
  ...
  router.put(
    "/business-units/:unitId/source-config/:type/:id",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true),
      ...
      if (definition.parentColumn) {
        ...
        if (!parent)
          return res
            .status(400)
            .json({ message: `${definition.label} parent is required` });
        sets.push(`${definition.parentColumn}=?`);
        values.push(parentId);
      }
      values.push(Number(req.params.id), unitId);
      const [result] = await pool.execute(
        `UPDATE ${definition.table} SET ${sets.join(",")} WHERE id=? AND business_unit_id=?`,
        values,
      );
      if (!result.affectedRows)
        return res
          .status(404)
          .json({ message: `${definition.label} not found` });
      res.json({ message: `${definition.label} updated successfully` });
    },
  );

  router.put(
    "/business-units/:unitId/source-config/:type/:id/status",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true),
      ...
      const [result] = await pool.execute(
        `UPDATE ${definition.table} SET is_active=? WHERE id=? AND business_unit_id=?`,
        [req.body.isActive ? 1 : 0, Number(req.params.id), unitId],
      );
      if (!result.affectedRows)
        return res
          .status(404)
          .json({ message: `${definition.label} not found` });
      res.json({
        message: `${definition.label} marked ${req.body.isActive ? "active" : "inactive"}`,
      });
    },
  );
  ...
  router.delete(
    ...
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        ...
        definition = sourceConfigDefinitions[type];
      if (!unit)
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      ...
      try {
        ...
        if (type === "channels") {
          const [rows] = await connection.execute(
            `SELECT id FROM crm_business_sources WHERE channel_id=? AND business_unit_id=?`,
            [id, unitId],
          );
        ...
        if (type === "channelCategories") {
          const [rows] = await connection.execute(
            `SELECT s.id FROM crm_business_sources s JOIN crm_business_channels c ON c.id=s.channel_id WHERE c.category_id=? AND c.business_unit_id=?`,
            [id, unitId],
          );
          sources.push(...rows.map((row) => Number(row.id)));
        }
        if (sources.length) {
          ...
          const [sourceRows] = await connection.execute(
            `SELECT source_key AS sourceKey FROM crm_business_sources WHERE id IN (${placeholders}) AND business_unit_id=?`,
            [...sources, unitId],
          );
          ...
          if (keys.length) {
            ...
            const [[linked]] = await connection.execute(
              `SELECT COUNT(*) AS count FROM crm_dynamic_leads WHERE business_unit_id=? AND source_key IN (${keyPlaceholders})`,
              [unitId, ...keys],
            );
            if (Number(linked.count) > 0) {
              ...
              return res
                .status(409)
                .json({
                  message: `Cannot delete this ${definition.label.toLowerCase()} because ${linked.count} lead${Number(linked.count) === 1 ? " is" : "s are"} linked to it`,
                });
            }
          }
        }
        if (type === "channelCategories") {
          await connection.execute(
            `DELETE s FROM crm_business_sources s JOIN crm_business_channels c ON c.id=s.channel_id WHERE c.category_id=? AND c.business_unit_id=?`,
            [id, unitId],
          );
          await connection.execute(
            `DELETE FROM crm_business_channels WHERE category_id=? AND business_unit_id=?`,
            [id, unitId],
          );
        } else if (type === "channels")
          await connection.execute(
            `DELETE FROM crm_business_sources WHERE channel_id=? AND business_unit_id=?`,
            [id, unitId],
          );
        else if (type === "campaignCategories")
          await connection.execute(
            `DELETE FROM crm_business_campaigns WHERE category_id=? AND business_unit_id=?`,
            [id, unitId],
          );
        const [result] = await connection.execute(
          `DELETE FROM ${definition.table} WHERE id=? AND business_unit_id=?`,
          [id, unitId],
        );
        if (!result.affectedRows) {
          await connection.rollback();
        ...
        await connection.commit();
        res.json({
          message: `${definition.label} and its unused child configuration deleted`,
        });
      } catch (error) {
        await connection.rollback();
        if (error.code === "ER_ROW_IS_REFERENCED_2")
          return res
            .status(409)
            .json({
              message: `This ${definition.label.toLowerCase()} is linked to application records and cannot be deleted`,
            });
        throw error;
      } finally {
        connection.release();
   ...
   */
  router.put(
    "/business-units/:id/duplicate-rule",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id);
      if (!(await accessibleUnit(req, unitId, true)))
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      ...
      if (!requested || !requested.length)
        return res
          .status(400)
          .json({
            message: "Choose at least one field for the duplicate rule",
          });
      ...
      if (dropped.length === requested.length)
        return res
          .status(400)
          .json({
            message: "None of those fields can be used in a duplicate rule",
          });
      await pool.execute(
        "UPDATE crm_business_units SET duplicate_rule_json=? WHERE id=?",
        [JSON.stringify(rule), unitId],
      );
      res.json({
        message: `Leads are now matched on ${describeRule(rule)}`,
        fields: rule,
        ignored: dropped,
      });
    },
  ...
  router.get(
    "/business-units/:id/lead-pipelines",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id);
      if (!(await accessibleUnit(req, unitId, true)))
      ...
      const [rows] = await pool.execute(
        `SELECT p.id,p.pipeline_key AS pipelineKey,p.display_name AS displayName,p.description,
         ...
         FROM crm_lead_pipelines p WHERE p.business_unit_id=? ORDER BY p.position,p.display_name`,
        [unitId],
      );
    ...
    async (req, res) => {
      ...
      if (!key)
        return res
          .status(400)
          .json({
            message: "Pipeline name must contain at least one letter or number",
          });
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_lead_pipelines WHERE business_unit_id=?`,
        [unitId],
      );
      // The first pipeline a unit ever gets is necessarily its default.
      const [[existing]] = await pool.execute(
        `SELECT COUNT(*) AS count FROM crm_lead_pipelines WHERE business_unit_id=?`,
        [unitId],
      );
      try {
        const [result] = await pool.execute(
        ...
        if (error.code === "ER_DUP_ENTRY")
          return res
            ...
            .json({
              message:
                "A pipeline with that name already exists in this business unit",
            });
        throw error;
      }
    },
  ...
  router.put(
    "/business-units/:unitId/lead-pipelines/:id",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        id = Number(req.params.id);
      ...
      const [[pipeline]] = await pool.execute(
        `SELECT * FROM crm_lead_pipelines WHERE id=? AND business_unit_id=?`,
        [id, unitId],
      );
      if (!pipeline)
        return res.status(404).json({ message: "Lead pipeline not found" });
      ...
      if (pipeline.is_default && (!isActive || req.body.isDefault === false)) {
        return res
          ...
          .json({
            message:
              "Make another pipeline the default before changing this one",
          });
      }
      const connection = await pool.getConnection();
      try {
        ...
        if (makeDefault)
          await connection.execute(
            `UPDATE crm_lead_pipelines SET is_default=FALSE WHERE business_unit_id=?`,
            [unitId],
          );
        await connection.execute(
          ...
          [
            ...
            req.body.description === undefined
              ...
              : text(req.body.description, 500) || null,
            isActive,
            makeDefault ? 1 : pipeline.is_default ? 1 : 0,
            id,
            unitId,
          ],
        );
    ...
    async (req, res) => {
      ...
      try {
        ...
        const [[locked]] = await connection.execute(
          `SELECT id FROM crm_lead_pipelines WHERE id=? AND business_unit_id=? FOR UPDATE`,
          [id, unitId],
        );
        if (!locked) {
          await connection.rollback();
        ...
        const [assignedLeads] = await connection.execute(
          ...
          WHERE st.pipeline_id=? FOR UPDATE`,
          [id],
        );
        if (assignedLeads.length > 0) {
          ...
          return res
            .status(409)
            .json({ message: `Cannot delete this pipeline because ${detail}` });
        }
        await connection.execute(
          `DELETE ss FROM crm_lead_substages ss
          JOIN crm_lead_stages st ON st.id=ss.stage_id
         WHERE st.pipeline_id=? AND st.business_unit_id=?`,
          [id, unitId],
        );
        const [stageResult] = await connection.execute(
          `DELETE FROM crm_lead_stages WHERE pipeline_id=? AND business_unit_id=?`,
          [id, unitId],
        );
        await connection.execute(
          `DELETE FROM crm_lead_pipelines WHERE id=? AND business_unit_id=?`,
          [id, unitId],
        );
        await connection.commit();
        res.json({
          message: `Lead pipeline deleted with ${Number(stageResult.affectedRows || 0)} unused stage${Number(stageResult.affectedRows || 0) === 1 ? "" : "s"}`,
        });
      } catch (error) {
        await connection.rollback();
        if (error.code === "ER_ROW_IS_REFERENCED_2")
          return res
            ...
            .json({
              message:
                "This pipeline is referenced by historical CRM records and cannot be deleted",
            });
        throw error;
      } finally {
        connection.release();
  ...
  router.post(
    "/business-units/:id/pipeline-stages",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        unit = await accessibleUnit(req, unitId, true),
        pipelineId = Number(req.body.pipelineId),
        displayName = text(req.body.displayName, 150),
        stageKey = slug(req.body.stageKey || displayName);
      if (!unit)
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      if (unit) {
        ...
        if (!pipeline)
          return res
            .status(400)
            .json({ message: "Choose a pipeline for this stage" });
        // Position runs within the pipeline, not across the whole unit.
        const [[next]] = await pool.execute(
          `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_lead_stages WHERE pipeline_id=?`,
          [pipeline.id],
        );
        ...
        const [[taken]] = await pool.execute(
          `SELECT COUNT(*) AS count FROM crm_lead_stages WHERE business_unit_id=? AND name=?`,
          [unitId, `bu${unitId}__${stageKey}`],
        );
        const uniqueKey = Number(taken.count)
          ? `bu${unitId}__${stageKey}_${pipeline.id}`
          : `bu${unitId}__${stageKey}`;
        const [result] = await pool.execute(
          ...
          [
            ...
            /^#[0-9a-f]{6}$/i.test(req.body.color || "")
              ? req.body.color
              : "#4A4FB1",
            req.body.requiresFollowup ? 1 : 0,
          ],
        );
        return res
          .status(201)
          .json({ id: Number(result.insertId), message: "Lead stage added" });
      }
      const [[pipeline]] = await pool.execute(
        `SELECT id FROM crm_metadata_pipelines WHERE id=? AND business_unit_id=?`,
        [pipelineId, unitId],
      );
      if (!pipeline || !displayName)
        return res
          .status(400)
          .json({ message: "Valid pipeline and stage name are required" });
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_metadata_pipeline_stages WHERE pipeline_id=?`,
        [pipelineId],
      );
      const [result] = await pool.execute(
        `INSERT INTO crm_metadata_pipeline_stages(pipeline_id,stage_key,display_name,stage_type,color_code,position) VALUES(?,?,?,?,?,?)`,
        [
          ...
          stageKey,
          displayName,
          ["open", "won", "lost", "on_hold"].includes(req.body.stageType)
            ? req.body.stageType
            : "open",
          /^#[0-9a-f]{6}$/i.test(req.body.color || "")
            ? req.body.color
      ...
      res
        .status(201)
        .json({ id: Number(result.insertId), message: "Pipeline stage added" });
    },
  );

  router.put(
    "/business-units/:unitId/pipeline-stages/:id",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true);
   ...
   */
  router.put(
    "/business-units/:unitId/pipeline-stages/:id/move",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        stageId = Number(req.params.id);
      if (!(await accessibleUnit(req, unitId, true)))
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      ...
      if (!direction)
        return res
          .status(400)
          .json({ message: "Select a valid move direction" });
      const connection = await pool.getConnection();
      try {
        await connection.beginTransaction();
        ...
        if (!adjacent) {
          await connection.rollback();
          return res.json({
            message: `Stage is already ${direction === "up" ? "first" : "last"}`,
          });
        }
        const [[parking]] = await connection.execute(
          `SELECT COALESCE(MAX(position),0)+1 AS slot FROM crm_lead_stages WHERE pipeline_id <=> ?`,
          [current.pipelineId],
        );
        await connection.execute(
          `UPDATE crm_lead_stages SET position=? WHERE id=?`,
          [Number(parking.slot), Number(current.id)],
        );
        await connection.execute(
          `UPDATE crm_lead_stages SET position=? WHERE id=?`,
          [Number(current.position), Number(adjacent.id)],
        );
        await connection.execute(
          `UPDATE crm_lead_stages SET position=? WHERE id=?`,
          [Number(adjacent.position), Number(current.id)],
        );
        await connection.commit();
   ...
   */
  router.put(
    "/business-units/:unitId/pipeline-substages/:id/move",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        substageId = Number(req.params.id);
      if (!(await accessibleUnit(req, unitId, true)))
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      ...
      try {
        ...
        const [[current]] = await connection.execute(
         ...
         WHERE ss.id=? AND s.business_unit_id=? FOR UPDATE`,
          [substageId, unitId],
        );
        if (!current) {
          await connection.rollback();
          return res.status(404).json({ message: "Lead sub-stage not found" });
        }
        ...
        if (index < 0 || target < 0 || target >= order.length) {
          await connection.rollback();
          return res.json({
            message: `Sub-stage is already ${direction === "up" ? "first" : "last"}`,
          });
        }
        [order[index], order[target]] = [order[target], order[index]];
        for (let position = 0; position < order.length; position += 1) {
          await connection.execute(
            `UPDATE crm_lead_substages SET position=? WHERE id=?`,
    ...
    async (req, res) => {
      ...
      if (unit) {
        const [[assigned]] = await pool.execute(
          `SELECT COUNT(*) AS count FROM crm_leads
         WHERE stage_id=? OR substage_id IN (SELECT id FROM crm_lead_substages WHERE stage_id=?)`,
          [stageId, stageId],
        );
        if (Number(assigned.count) > 0)
          return res
            .status(409)
            .json({
              message: `Cannot delete this stage because ${assigned.count} lead${Number(assigned.count) === 1 ? " is" : "s are"} assigned to it`,
            });
        const connection = await pool.getConnection();
        try {
          await connection.beginTransaction();
          const [[stage]] = await connection.execute(
            `SELECT id FROM crm_lead_stages WHERE id=? AND business_unit_id=? FOR UPDATE`,
            [stageId, unitId],
          );
          if (!stage) {
            await connection.rollback();
          ...
          await connection.execute(
            `UPDATE crm_lead_substages SET is_active=FALSE WHERE stage_id=?`,
            [stageId],
          );
          await connection.execute(
            `UPDATE crm_lead_stages SET is_active=FALSE WHERE id=? AND business_unit_id=?`,
            [stageId, unitId],
          );
          await connection.commit();
          return res.json({
            message: "Lead stage and its unused sub-stages deleted",
          });
        } catch (error) {
          await connection.rollback();
          if (error.code === "ER_ROW_IS_REFERENCED_2")
            return res
              ...
              .json({
                message:
                  "This stage is part of existing lead history and cannot be deleted; mark it inactive instead",
              });
          throw error;
        } finally {
          connection.release();
      ...
      const [[assigned]] = await pool.execute(
       ...
       JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id
       WHERE dl.stage_id=? AND p.business_unit_id=? AND dl.deleted_at_utc IS NULL`,
        [stageId, unitId],
      );
      if (Number(assigned.count) > 0)
        return res
      ...
      try {
        ...
        if (error.code === "ER_ROW_IS_REFERENCED_2")
          return res
            ...
            .json({
              message:
                "This stage is used by existing records or workflow history and cannot be deleted; mark it inactive instead",
            });
        throw error;
      }
    },
  ...
  router.post(
    "/business-units/:id/pipeline-substages",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
        unit = await accessibleUnit(req, unitId, true);
      ...
      const stageId = Number(req.body.stageId),
        displayName = text(req.body.displayName, 150),
        substageKey = slug(req.body.substageKey || displayName);
      if (!stageId || !displayName)
        return res
          .status(400)
          .json({ message: "Parent stage and sub-stage name are required" });
      if (unit) {
        const [[stage]] = await pool.execute(
          `SELECT id FROM crm_lead_stages WHERE id=? AND business_unit_id=?`,
          [stageId, unitId],
        );
        if (!stage)
          return res.status(400).json({ message: "Select a valid lead stage" });
        const [[next]] = await pool.execute(
          `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_lead_substages WHERE stage_id=?`,
          [stageId],
        );
        const [result] = await pool.execute(
      ...
      if (!stage)
        return res
          .status(400)
          .json({ message: "Select a valid pipeline stage" });
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_metadata_pipeline_substages WHERE stage_id=?`,
        [stageId],
      );
      const [result] = await pool.execute(
      ...
      res
        ...
        .json({
          id: Number(result.insertId),
          message: "Pipeline sub-stage added",
        });
    },
  );

  router.put(
    "/business-units/:unitId/pipeline-substages/:id",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true);
      ...
      if (!result.affectedRows)
        return res
          .status(404)
          .json({ message: "Pipeline sub-stage not found" });
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      if (unit) {
        const [[assigned]] = await pool.execute(
          `SELECT COUNT(*) AS count FROM crm_leads WHERE substage_id=?`,
          [substageId],
        );
        if (Number(assigned.count) > 0)
          return res
            .status(409)
            .json({
              message: `Cannot delete this sub-stage because ${assigned.count} lead${Number(assigned.count) === 1 ? " is" : "s are"} assigned to it`,
            });
        try {
          // Soft deletion records the administrator's choice so replayable seed
          // migrations cannot recreate the sub-stage later.
          const [result] = await pool.execute(
            `UPDATE crm_lead_substages ss JOIN crm_lead_stages s ON s.id=ss.stage_id SET ss.is_active=FALSE WHERE ss.id=? AND s.business_unit_id=?`,
            [substageId, unitId],
          );
          if (!result.affectedRows)
            return res
              .status(404)
              .json({ message: "Lead sub-stage not found" });
          ...
          if (error.code === "ER_ROW_IS_REFERENCED_2")
            return res
              ...
              .json({
                message:
                  "This sub-stage is part of existing lead history and cannot be deleted; mark it inactive instead",
              });
          throw error;
        }
      }
  ...
  router.post(
    "/business-units/:id/operation-stages",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.id),
      ...
      const [[workflow]] = await pool.execute(
        `SELECT id FROM crm_operation_workflows WHERE id=? AND business_unit_id=?`,
        [workflowId, unitId],
      );
      if (!workflow || !displayName)
        return res
          .status(400)
          .json({ message: "Valid workflow and stage name are required" });
      const [[next]] = await pool.execute(
        `SELECT COALESCE(MAX(position),0)+1 AS position FROM crm_operation_stages WHERE workflow_id=?`,
        [workflowId],
      );
      const [result] = await pool.execute(
        ...
        [
          ...
          stageKey,
          displayName,
          ["open", "completed", "cancelled", "on_hold"].includes(
            req.body.stageType,
          )
            ? req.body.stageType
            : "open",
          /^#[0-9a-f]{6}$/i.test(req.body.color || "")
            ...
            : "#4A4FB1",
          Number(next.position),
          JSON.stringify(req.body.checklist || []),
        ],
      );
      res
        .status(201)
        .json({ id: Number(result.insertId), message: "Tracker stage added" });
    },
  );

  router.put(
    "/business-units/:unitId/operation-stages/:id",
    requireUserAdmin,
    async (req, res) => {
      const stageType = ["open", "completed", "cancelled", "on_hold"].includes(
        req.body.stageType,
      )
        ? req.body.stageType
        : "open";
      ...
      if (!result.affectedRows)
        return res.status(404).json({ message: "Tracker stage not found" });
      res.json({ message: "Tracker stage updated" });
    },
  );

  router.put(
    "/business-units/:unitId/operation-stages/:id/move",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        stageId = Number(req.params.id);
      ...
      try {
        ...
        if (!current) {
          await connection.rollback();
          return res.status(404).json({ message: "Tracker status not found" });
        }
        const comparator = direction === "up" ? "<" : ">",
          order = direction === "up" ? "DESC" : "ASC";
        const [[adjacent]] = await connection.execute(
        ...
        if (!adjacent) {
          await connection.rollback();
          return res.json({
            message: `Tracker status is already ${direction === "up" ? "first" : "last"}`,
          });
        }
        await connection.execute(
          `UPDATE crm_operation_stages SET position=? WHERE id=?`,
          [Number(adjacent.position), Number(current.id)],
        );
        await connection.execute(
          `UPDATE crm_operation_stages SET position=? WHERE id=?`,
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      const [[linked]] = await pool.execute(
       ...
       WHERE opr.stage_id=? AND ow.business_unit_id=?`,
        [stageId, unitId],
      );
      if (Number(linked.count) > 0)
        return res
          .status(409)
          .json({
            message: `Cannot delete this Tracker stage because ${linked.count} record${Number(linked.count) === 1 ? " is" : "s are"} assigned to it`,
          });
      const [result] = await pool.execute(
  ...
  router.get("/business-units/:id/dynamic-leads", async (req, res) => {
    ...
    const search = text(req.query.search, 100),
      pipelineId = Number(req.query.pipelineId) || null,
      stageId = Number(req.query.stageId) || null;
    const values = [unitId],
      where = ["dl.business_unit_id=?", "dl.deleted_at_utc IS NULL"];
    if (search) {
      where.push(
        `(dl.display_name LIKE ? OR dl.primary_phone LIKE ? OR dl.primary_email LIKE ? OR dl.lead_number LIKE ?)`,
      );
      values.push(...Array(4).fill(`%${search}%`));
    }
    if (pipelineId) {
      where.push("dl.pipeline_id=?");
      values.push(pipelineId);
    }
    if (stageId) {
      where.push("dl.stage_id=?");
      values.push(stageId);
    }
    const [rows] = await pool.execute(
    ...
    res.json({
      data: rows.map((row) => ({
        ...row,
        id: Number(row.id),
        customValues: parseJson(row.customValues, {}),
      })),
    });
  });
  ...
  router.post("/business-units/:id/dynamic-leads", async (req, res) => {
    ...
    const pipelineId = Number(req.body.pipelineId),
      ...
      displayName = text(req.body.displayName, 200);
    if (!displayName)
      return res.status(400).json({ message: "Lead name is required" });
    const [[stage]] = await pool.execute(
      `SELECT ps.id FROM crm_metadata_pipeline_stages ps JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id
       WHERE ps.id=? AND p.id=? AND p.business_unit_id=? AND ps.is_active=TRUE`,
      [stageId, pipelineId, unitId],
    );
    if (!stage)
      return res.status(400).json({ message: "Select a valid pipeline stage" });
    ...
    const [result] = await pool.execute(
      ...
      [
        ...
        text(req.body.email, 254),
        text(req.body.source, 80),
        text(req.body.status, 80) || "active",
        JSON.stringify(req.body.customValues || {}),
        Number(req.user.id),
      ],
    );
    const leadNumber = `${String(unit.unit_code).slice(0, 4).toUpperCase()}-${new Date().getFullYear()}-${String(result.insertId).padStart(6, "0")}`;
    await pool.execute(
      `UPDATE crm_dynamic_leads SET lead_number=? WHERE id=?`,
  ...
  router.put(
    "/business-units/:unitId/dynamic-leads/:id/stage",
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true);
      if (!unit)
      ...
      const [[stage]] = await pool.execute(
        `SELECT ps.id FROM crm_metadata_pipeline_stages ps JOIN crm_metadata_pipelines p ON p.id=ps.pipeline_id
       WHERE ps.id=? AND p.business_unit_id=? AND ps.is_active=TRUE`,
        [stageId, unitId],
      );
      if (!stage)
  ...
  async function resolveTrackerOwner(connection, unitId, body, userId) {
    ...
    if (guestOwnerId) {
      ...
      if (!guest)
        throw Object.assign(new Error("Selected task owner is not available"), {
          statusCode: 400,
        });
    ...
    throw Object.assign(new Error("Task owner is required"), {
      statusCode: 400,
    });
  }
  ...
  const flushTrackerNotifications = async () => {
    ...
    for (const task of queued) {
      if (integrationHubService)
        await notifyTrackerTask(pool, integrationHubService, task, console);
    }
  };
   ...
   */
  router.get(
    "/business-units/:unitId/tracker-notifications",
    requireUserAdmin,
    async (req, res) => {
      const unitId = Number(req.params.unitId);
  ...
  router.put(
    ...
    async (req, res) => {
      const unitId = Number(req.params.unitId);
      const enabled =
        req.body.isEnabled === false ? 0 : req.body.isEnabled ? 1 : 0;
      ...
      if (enabled && !integrationId)
        return res
          .status(400)
          .json({
            message: "Choose the WhatsApp account these messages send from",
          });
      if (
        enabled &&
        !text(req.body.actionItemTemplate, 200) &&
        !text(req.body.approvalTemplate, 200)
      ) {
        return res
          .status(400)
          .json({ message: "Choose at least one template" });
      }
      await pool.execute(
  ...
  async function insertTrackerTask(
    connection,
    { unitId, unit, userId, body, index = null },
  ) {
    ...
    const [[stage]] = await connection.execute(
      `SELECT os.id FROM crm_operation_stages os JOIN crm_operation_workflows ow ON ow.id=os.workflow_id
       WHERE os.id=? AND ow.id=? AND ow.business_unit_id=?`,
      [stageId, workflowId, unitId],
    );
    if (!stage || !title)
      throw Object.assign(
        new Error(`${prefix}title, tracker, and status are required`),
        { statusCode: 400 },
      );
    if (!body.dueAt)
      throw Object.assign(new Error(`${prefix}deadline is required`), {
        statusCode: 400,
      });
    const owner = await resolveTrackerOwner(connection, unitId, body, userId);
    const approverUserIds = [
      ...new Set(
        (body.approverUserIds || []).map(Number).filter(Number.isInteger),
      ),
    ];
    const approvalRequired = Boolean(body.approvalRequired);
    if (approvalRequired && !approverUserIds.length)
      throw Object.assign(new Error(`${prefix}select at least one approver`), {
        statusCode: 400,
      });
    ...
    const [result] = await connection.execute(
      ...
      [
        ...
        workflowId,
        stageId,
        body.dynamicLeadId ? Number(body.dynamicLeadId) : null,
        body.legacyLeadId ? Number(body.legacyLeadId) : null,
        temporary,
        title,
    ...
    await connection.execute(
      `UPDATE crm_operation_records SET record_number=? WHERE id=?`,
    ...
    await notifyEmployee(connection, {
      businessUnitId: unitId,
      employeeId: owner.ownerEmployeeId,
      actorUserId: userId,
      type: "action_assigned",
      title: "Action item assigned to you",
      message: `${title} · ${recordNumber}`,
      link: "/tracker",
    ...
    if (Number(body.minutesSpent) > 0)
      await connection.execute(
        ...
        [
          ...
          text(body.timeNote, 1000) || "Initial time recorded",
          userId,
        ],
      );
  ...
  router.get("/business-units/:id/mom-sessions", async (req, res) => {
    ...
    const operationIds = [
      ...new Set(
        points.map((point) => Number(point.operationRecordId)).filter(Boolean),
      ),
    ];
    const [pointApprovals] = operationIds.length
      ? await pool.execute(
    ...
    res.json({
      data: sessions.map((session) => {
        const sessionPoints = points
          .filter((point) => Number(point.sessionId) === Number(session.id))
          .map((point) => ({
            ...point,
            id: Number(point.id),
            sessionId: Number(point.sessionId),
            operationRecordId: point.operationRecordId
              ? Number(point.operationRecordId)
              : null,
            approvalDetails: pointApprovals
              .filter(
                (approval) =>
                  Number(approval.operationRecordId) ===
                  Number(point.operationRecordId),
              )
              .map((approval) => ({
                ...approval,
                id: Number(approval.id),
                operationRecordId: Number(approval.operationRecordId),
              })),
          }));
        return {
          ...session,
          id: Number(session.id),
          actionItemCount: Number(session.actionItemCount),
          notes:
            session.notes ||
            sessionPoints
              .map((point) => point.notes)
              .filter(Boolean)
              .join("\n\n"),
  ...
  router.get("/business-units/:id/tracker-approvals", async (req, res) => {
    ...
    res.json({
      data: rows.map((row) => ({
        ...
        documentReferences: parseJson(row.documentReferences, []),
      })),
    });
  });
  ...
  router.get("/business-units/:unitId/operations/:id", async (req, res) => {
    ...
    if (!record)
      return res.status(404).json({ message: "Tracker task not found" });
    ...
    res.json({
      data: {
        ...record,
        id: Number(record.id),
        values: parseJson(record.values_json, {}),
        timeLogs,
        approvals: approvals.map((item) => ({
          ...item,
          documentReferences: parseJson(item.documentReferences, []),
        })),
      },
    });
  });
  ...
  router.post("/business-units/:id/operations", async (req, res) => {
    ...
    try {
      await connection.beginTransaction();
      const created = await insertTrackerTask(connection, {
        unitId,
        unit,
  ...
  router.post("/business-units/:id/operations/batch", async (req, res) => {
    ...
    if (!unit)
      return res
        .status(403)
        .json({ message: "Business unit management access required" });
    const items = (Array.isArray(req.body.items) ? req.body.items : []).filter(
      (item) => text(item.title, 200),
    );
    const momNotes = richText(req.body.momNotes, 50000);
    if (!momNotes)
      return res.status(400).json({ message: "MOM notes are required" });
    if (items.length > 100)
      return res
        .status(400)
        .json({
          message: "A meeting session can contain up to 100 action items",
        });
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();
      ...
      if (requestedSessionId) {
        ...
        if (!existingSession)
          throw Object.assign(new Error("MOM session not found"), {
            statusCode: 404,
          });
        sessionId = Number(existingSession.id);
        sessionNumber = existingSession.sessionNumber;
        existingActionCount = Number(existingSession.actionItemCount || 0);
        const [[position]] = await connection.execute(
          `SELECT COALESCE(MAX(position),0)+1 AS nextPosition FROM crm_mom_session_points WHERE session_id=?`,
          [sessionId],
        );
        nextPosition = Number(position.nextPosition);
        await connection.execute(
          `UPDATE crm_mom_sessions SET mom_notes=?,ended_at_utc=CURRENT_TIMESTAMP(6) WHERE id=?`,
          [momNotes, sessionId],
        );
      } else {
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        if (!Number.isFinite(estimatedHours) || estimatedHours <= 0)
          throw Object.assign(
            new Error(
              `Action item ${index + 1}: estimated duration is required`,
            ),
            { statusCode: 400 },
          );
        const calculatedDueAt = new Date(
        ...
        const operation = await insertTrackerTask(connection, {
          ...
          body: {
            ...item,
            dueAt: item.dueAt || calculatedDueAt,
            workflowId: Number(defaults.workflowId),
            stageId: Number(defaults.stageId),
          },
        });
        ...
        await connection.execute(
          ...
          [
            sessionId,
            nextPosition + index,
            text(item.description, 10000) || text(item.title, 200),
            operation.id,
          ],
        );
      }
  ...
  router.put("/business-units/:unitId/operations/:id", async (req, res) => {
    ...
    if (!unit)
      return res
        ...
        .json({ message: "Business unit management access required" });
    const stageId = Number(req.body.stageId);
    if (!text(req.body.title, 200) || !stageId || !req.body.dueAt)
      return res
        .status(400)
        .json({ message: "Title, status, owner, and deadline are required" });
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();
      const owner = await resolveTrackerOwner(
        connection,
        unitId,
        req.body,
        Number(req.user.id),
      ...
      if (
        Number(owner.ownerEmployeeId) &&
        Number(owner.ownerEmployeeId) !==
          Number(previousOperation?.ownerEmployeeId)
      )
        await notifyEmployee(connection, {
          businessUnitId: unitId,
          employeeId: owner.ownerEmployeeId,
          ...
          type: "action_assigned",
          title: "Action item assigned to you",
          message: text(req.body.title, 200),
          link: "/tracker",
          entityType: "operation",
          entityId: Number(req.params.id),
        });
      await connection.commit();
      res.json({ message: "Tracker task updated" });
    } catch (error) {
      await connection.rollback();
      if (error.statusCode)
        return res.status(error.statusCode).json({ message: error.message });
  ...
  router.post(
    "/business-units/:unitId/operations/:id/time-logs",
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true),
        minutes = Number(req.body.minutesSpent);
      if (!unit)
        return res
          .status(403)
          .json({ message: "Business unit management access required" });
      if (
        ...
        minutes <= 0 ||
        !text(req.body.workNote, 1000)
      )
        return res
          .status(400)
          .json({ message: "Positive minutes and a work note are required" });
      const connection = await pool.getConnection();
      try {
        await connection.beginTransaction();
        const [updated] = await connection.execute(
          `UPDATE crm_operation_records SET minutes_spent=minutes_spent+?,updated_by_user_id=? WHERE id=? AND business_unit_id=?`,
          [minutes, Number(req.user.id), Number(req.params.id), unitId],
        );
        ...
        await connection.commit();
        res.status(201).json({ message: "Work minutes recorded" });
      } catch (error) {
        await connection.rollback();
        throw error;
      } finally {
  ...
  router.put(
    "/business-units/:unitId/tracker-approvals/:id/decision",
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId),
      ...
      if (!["approved", "rejected"].includes(decision))
        return res.status(400).json({ message: "Select approve or reject" });
      if (!remarks || remarks.length < 10)
        return res
          .status(400)
          .json({
            message: "Provide a clear explanation of at least 10 characters",
          });
      const connection = await pool.getConnection();
      try {
        await connection.beginTransaction();
        ...
        const [result] = await connection.execute(
         ...
         WHERE oa.id=? AND (oa.approver_user_id=? OR ?=1) AND opr.business_unit_id=? AND oa.decision='pending'`,
          [
            decision,
            remarks,
            JSON.stringify((req.body.documentReferences || []).filter(Boolean)),
            Number(req.user.id),
            Number(req.params.id),
            Number(req.user.id),
            admin ? 1 : 0,
        ...
        if (!result.affectedRows) {
          ...
          return res
            .status(404)
            .json({ message: "Pending approval request not found" });
        }
        ...
        const [[summary]] = await connection.execute(
          `SELECT SUM(decision='rejected') rejected,SUM(decision='pending') pending FROM crm_operation_approvals WHERE operation_record_id=?`,
          [Number(approval.recordId)],
        );
        const overall =
          Number(summary.rejected) > 0
            ...
            : Number(summary.pending) > 0
              ...
              : "approved";
        await connection.execute(
          `UPDATE crm_operation_records SET approval_status=?,updated_at_utc=CURRENT_TIMESTAMP(6) WHERE id=?`,
          [overall, Number(approval.recordId)],
        );
        if (decision === "approved") {
          const notice = {
            ...
            message: `${approval.title} · ${approval.recordNumber}`,
            link: "/tracker",
            entityType: "operation",
            entityId: Number(approval.recordId),
          };
          await notifyEmployee(connection, {
            ...notice,
            employeeId: Number(approval.ownerEmployeeId),
          });
          await notifyUser(connection, {
            ...notice,
            userId: Number(approval.requestedByUserId),
          });
        }
        await connection.commit();
        res.json({ message: `Task ${decision}` });
      } catch (error) {
        await connection.rollback();
        throw error;
      } finally {
  ...
  router.put(
    "/business-units/:unitId/operations/:id/stage",
    async (req, res) => {
      const unitId = Number(req.params.unitId),
        unit = await accessibleUnit(req, unitId, true);
      if (!unit)
      ...
      if (!result.affectedRows)
        return res.status(404).json({ message: "Tracker record not found" });
      res.json({ message: "Tracker stage updated" });
    },
  );
