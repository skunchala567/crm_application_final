...
async function branchConfig(pool,id){
  const [[branch]]=await pool.execute(`SELECT id,branch_name AS name,jodo_payment_enabled AS enabled,jodo_api_key AS apiKey,jodo_secret_key AS secretKey,jodo_collector_code AS collectorCode,jodo_base_url AS baseUrl,jodo_auth_header AS authHeader,application_payment_component AS paymentComponent FROM branches WHERE id=? AND is_active=1 LIMIT 1`,[id]);
...
export function createJodoPaymentLinkRoutes(pool,authenticate,requireCrmAccess,requireUserAdmin){
  ...
  router.get('/collections/report',canView,wrap(async(req,res)=>{
    ...
    if(req.query.search){where.push('(x.orderId LIKE ? OR x.transactionId LIKE ? OR x.payerName LIKE ? OR x.payerEmail LIKE ? OR x.leadNumber LIKE ?)');const q=`%${clean(req.query.search,120)}%`;params.push(q,q,q,q,q);}
    const [rows]=await pool.execute(`SELECT x.* FROM (
      ...
      SELECT s.business_unit_id,pf.branch_id,b.branch_name,NULL,NULL,'payment_form',s.jodo_order_id,s.transaction_id,s.payer_name,s.payer_email,s.payer_phone,
       s.amount,s.status,s.paid_at_utc,s.settled_at_utc,s.settlement_utr,s.created_at_utc
      ...
      SELECT p.business_unit_id,p.branch_id,b.branch_name,p.lead_id,l.lead_number,'payment_link',p.order_id,p.transaction_id,p.payer_name,p.payer_email,p.payer_phone,
       p.amount,p.status,p.paid_at_utc,p.settled_at_utc,p.settlement_utr,p.created_at_utc
